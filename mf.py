...
def get_funds_url_mf(xlsx_path: str) -> list[dict]:
    ...
    for i in range(total):
        ...
        data = get_funds_json(headers=headers, cookies=cookies, start=start, rpp=rpp)
...
def get_fund_keyword_mf(driver: WebDriver, funds: list[dict]) -> list[dict]:
    ...
    for fund in funds:
        if i % 20 == 0:
            print(f"Processing Keywords Batch [{i}/{len(funds)}]")
        try:
            name = fund["name"]
            ...
            f = dict(
                name=name, isin=isin, url=url, keyword=keyword, index=fund.get("index")
            )
